...
import 'package:chef/models/signup/sign_up_questionnaire_request.dart'
    as baserequest;
...
@injectable
class SignUpQuestionnaireScreenViewModel
    extends BaseViewModel<SignUpQuestionnaireState> {
  ...
  Future<bool?> uploadFoodieImage({file, required String baseUrl}) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        await locateService<IStorageService>().writeString(
            key: 'profile_image',
            data: profileImageResponse.t!.profileImageUrl.toString());
        print('success');
  ...
  Future<void> getQuestionnaireData(bool isProfileUpdate,
      {required String userId}) async {
    final url = InfininURLHelpers.getRestApiURL(
        Api.baseURL + Api.singUpQuestionnaireList);
  ...
  void addModelsFromQuestions(
      {Function? completion, required BuildContext context}) {
    if (answerIdsUniqueFoodie.isNotEmpty &&
        answerIdPerfectAmbience.isNotEmpty &&
        answerIdsCuisineTaste.isNotEmpty &&
        answerIdInterest.isNotEmpty) {
      foodieQuestionAnswersList.add(T1(
          answerIds: answerIdsUniqueFoodie,
          foodieId: _appService.state.userInfo!.t.id,
          id: 1,
          inputAnswer: '',
          questionId: 1));
      foodieQuestionAnswersList.add(T1(
          answerIds: answerIdPerfectAmbience,
          foodieId: _appService.state.userInfo!.t.id,
          id: 2,
          inputAnswer: '',
          questionId: 2));
      foodieQuestionAnswersList.add(T1(
          answerIds: answerIdsCuisineTaste,
          foodieId: _appService.state.userInfo!.t.id,
          id: 12,
          inputAnswer: '',
          questionId: 12));
      foodieQuestionAnswersList.add(T1(
          answerIds: answerIdInterest,
          foodieId: _appService.state.userInfo!.t.id,
          id: 13,
          inputAnswer: '',
          questionId: 13));
  ...
  Future<void> saveFoodieQuestionAnswerAndProfileImage({
    ...
    final url =
        InfininURLHelpers.getRestApiURL(baseUrl + Api.saveFoodieAnswers);
    ...
    if (response != null) {
      ...
      FoodieQuestionAnswerResponse foodieQuestionAnswerResponse =
          foodieQuestionAnswerResponseFromJson(response.body);
  ...
  Future<void> getFoodieAnswers() async {
    ...
    final response = await _network.post(
      path: url,
      data: {"t": _appService.state.userInfo!.t.id},
      header: {
        'Authorization': 'Bearer ${_storage.readString(key: 'auth_token')}',
        'Content-Type': 'application/json'
      },
