...
class BookingListScreen extends BaseView<BookingListScreenViewModel> {
  ...
  Widget buildScreen({required BuildContext context, required ScreenSizeData screenSizeData}) {
    ...
    return BlocBuilder<BookingListScreenViewModel,
        BookingListState>(
        bloc: viewModel..getBookingListData(userId: '45'),
        builder: (context, state) {
          return Scaffold(
            backgroundColor: appTheme.colors.primaryBackground,
            body: state.when(loading: _loading, loaded: (bookingListModel) => displayLoaded(bookingListModel)),
          );
        });
  }
